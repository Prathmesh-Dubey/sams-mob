...
(function () {
  ...
  /* Lower = smoother, longer glide. 0.22 was the original snappier value. */
  var SMOOTH_RATE = 0.20;
  ...
  function frame(now) {
    ...
    if (hero) {
      ...
      else if (Math.abs(p - sp) > 0.00012) { sp = chase(sp, p, SMOOTH_RATE, dt); moving = true; }
      else sp = p;
      render();
    }
    ...
    if (product && rail) {
      ...
      else if (Math.abs(rp - srp) > 0.00012) { srp = chase(srp, rp, SMOOTH_RATE, dt); moving = true; }
      else srp = rp;
