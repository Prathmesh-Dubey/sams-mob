...
(function () {
  ...
  /* A fixed key light, up and to the left and mostly in front of the camera.
     Faces are shaded against it every frame, which is what stops a box of six
     flat photographs from reading as a box of six flat photographs the moment
     it turns off-axis.

     The range is deliberately narrow - roughly 0.88 to 1.06. Each render is
     already correctly lit on its own, so this only has to separate one face
     from the next; a wide range turns an unlit side into a black card stuck
     to the side of a lit phone. */
  var LIGHT   = [-0.22, -0.38, 0.90];
  var AMBIENT = 0.88;
  var KEY     = 0.18;
  ...
  var title = el('h2', 'hc__title', fullTitle);
  ...
  function turnAt(p) {
    if (p <= TURN[0][0]) { return [TURN[0][1], TURN[0][2]]; }
    for (var i = 1; i < TURN.length; i++) {
      if (p <= TURN[i][0]) {
        var a = TURN[i - 1], b = TURN[i];
        var span = b[0] - a[0];
        var t = span > 0 ? smooth((p - a[0]) / span) : 1;
  ...
  /* CSS applies a transform list right to left, so rotateX(rx) rotateY(ry)
     turns a normal through Y first and then through X. */
  function rotY(v, c, s) { return [v[0] * c + v[2] * s, v[1], -v[0] * s + v[2] * c]; }
  function rotX(v, c, s) { return [v[0], v[1] * c - v[2] * s, v[1] * s + v[2] * c]; }

  var shown = -1;

  function paintModel(ry, rx) {
    model.style.setProperty('--ry', ry.toFixed(2) + 'deg');
    model.style.setProperty('--rx', rx.toFixed(2) + 'deg');

    var yr = ry * Math.PI / 180, xr = rx * Math.PI / 180;
    var cy = Math.cos(yr), sy = Math.sin(yr);
    var cx = Math.cos(xr), sx = Math.sin(xr);

    /* One pass: shade every face against the key light, and note which one
       is most square to the camera so the caption always names the face the
       viewer is actually looking at - including part way through a turn. */
    var best = 0, bestZ = -2;
    for (var i = 0; i < FACES.length; i++) {
      var n = rotX(rotY(FACES[i].n, cy, sy), cx, sx);
      if (n[2] > bestZ) { bestZ = n[2]; best = i; }
      var lit = n[0] * LIGHT[0] + n[1] * LIGHT[1] + n[2] * LIGHT[2];
      if (lit < 0) { lit = 0; }
      FACES[i].el.style.filter = 'brightness(' + (AMBIENT + KEY * lit).toFixed(3) + ')';
    }

    if (best !== shown) {
      shown = best;
      face.textContent = ('0' + (best + 1)).slice(-2) + ' / 06  ' + FACES[best].label;
      desc.textContent = FACES[best].desc;
    }
  }
